...
{
	class Elaborar_formula
	{
		private StringBuilder ticket = new StringBuilder();

		public void construccion_ticket(DTO_Formula formula_rest, List<DTO_Detallado_formula_elaborada> articulos_local, bool terminado = false)
		{
			DAO_Formulas dao_formula = new DAO_Formulas();
			DAO_Empleados dao_empleados = new DAO_Empleados();

			var informacion_formula = dao_formula.get_informacion_formula(formula_rest.formula_id);
			var informacion_empleado = dao_empleados.get_empleado_data((int)informacion_formula.empleado_id);
			var detallado = dao_formula.get_detallado_formulas(formula_rest.formula_id);
			...
			foreach (DTO_Detallado_formulas det_ticket in detallado)
			{
				...
				foreach(DTO_Detallado_formula_elaborada articulo_disponible in articulos_local)
				{
					if(articulo_disponible.articulo_id == det_ticket.articulo_id)
					{
						ticket.AppendLine(string.Format("{0,5} {1,-9} {2,26} {3,13}",
								"",
								articulo_disponible.caducidad,
								(articulo_disponible.lote.Equals(" ")) ? Misc_helper.PadBoth("SIN LOTE",26) : Misc_helper.PadBoth(articulo_disponible.lote,26),
								articulo_disponible.cantidad.ToString("#.##")
							)
						);
					}	
				}
			...
			if (informacion_formula.comentarios.Trim().Length > 0 && informacion_formula.comentarios_elaboracion.Trim().Length > 0)
			{
				ticket.AppendLine();
				ticket.AppendLine(informacion_formula.comentarios_elaboracion);
			}
			else if (informacion_formula.comentarios_elaboracion.Trim().Length > 0)
			{
				ticket.AppendLine("--------------------------------------------------------");
				//ticket.AppendLine(POS_Control.font_size_32 + POS_Control.align_center);
                ticket.Append("[FONT_SIZE_32][ALIGN_CENTER]");
            ...
            ticket.AppendLine(POS_Control.barcode(string.Format("{0}${1}", informacion_formula.sucursal_id.ToString().PadLeft(3, '0'), informacion_formula.sucursal_folio.ToString().PadLeft(3, '0'))));
		...
		public bool print()
		{
            return HELPERS.Print_new_helper.print((long)Misc_helper.get_terminal_id(), ticket.ToString(), "ELABORAR_FORMULA", 0, true);
		}
	}
}
