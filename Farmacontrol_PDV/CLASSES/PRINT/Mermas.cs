...
namespace Farmacontrol_PDV.CLASSES.PRINT
{
	class Mermas
	{
		private long merma_id;
		private StringBuilder ticket = new StringBuilder();

		public void construccion_ticket(long merma_id, bool reimpresion = false)
		{
			this.merma_id = merma_id;

			DAO_Mermas dao_mermas = new DAO_Mermas();
			var merma_ticket = dao_mermas.get_informacion_merma(merma_id);
			var detallado_merma = dao_mermas.get_detallado_merma_ticket(merma_id);
			...
			ticket.AppendLine(string.Format("FOLIO: #{0}", merma_ticket.merma_id));
			ticket.AppendLine(string.Format("CREADO POR: {0}", merma_ticket.nombre_empleado_captura.ToUpper()));
			ticket.AppendLine(string.Format("TERMINADO POR: {0}", merma_ticket.nombre_empleado_termina.ToUpper()));
            ticket.AppendLine(string.Format("FECHA CREADO: {0}", (merma_ticket.fecha_creado != null) ? Misc_helper.fecha(merma_ticket.fecha_creado.ToString(), "LEGIBLE") : " - "));
            ticket.AppendLine(string.Format("FECHA TERMINADO: {0}", (merma_ticket.fecha_terminado != null) ? Misc_helper.fecha(merma_ticket.fecha_terminado.ToString(), "LEGIBLE") : " - "));
			...
			foreach (DTO_Detallado_merma detallado_ticket in detallado_merma)
			{
		...
		public bool print()
		{
            return HELPERS.Print_new_helper.print((long)Misc_helper.get_terminal_id(), ticket.ToString(), "MERMA", merma_id, true);
		}
