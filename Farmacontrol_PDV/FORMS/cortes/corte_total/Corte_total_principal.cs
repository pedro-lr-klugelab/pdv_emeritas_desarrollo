...
namespace Farmacontrol_PDV.FORMS.cortes.corte_total
{
    public partial class Corte_total_principal : Form
    {
        DAO_Cortes dao_cortes = new DAO_Cortes();
        DTO_Corte dto_corte = new DTO_Corte();
        DTO_Corte dto_corte_total = new DTO_Corte();
        DTO_Corte ultimo_corte = new DTO_Corte();
        DTO_Corte dto_ultimo_corte_parcial = new DTO_Corte();
        DTO_Corte dto_ultimo_corte_total = new DTO_Corte();
        DAO_Entradas dao_entradas = new DAO_Entradas();
        private decimal tipoCambioDolar = 0m;
        public Corte_total_principal()
        {
            InitializeComponent();
        ...
        public void get_informacion_corte()
        {
            ...
            lbl_folio_ventas_parcial.Text = "Ventas del corte del folio #" + dto_corte.venta_inicial + " - #" + dto_corte.venta_final;
            ...
            lbl_total_parcial.Text = dto_corte.importe_total.ToString("C2");
            ...
            dto_ultimo_corte_parcial = dao_cortes.get_ultimo_corte();

            if (dto_ultimo_corte_parcial.corte_id > 0)
            {
                lbl_fecha_ultimo_corte.Text = (dto_ultimo_corte_parcial.fecha != null) ? Misc_helper.fecha(dto_ultimo_corte_parcial.fecha.ToString(), "LEGIBLE") : "AUN SIN CORTES";
                lbl_folio_ultimo_corte.Text = (dto_ultimo_corte_parcial.corte_id > 0) ? "#" + dto_ultimo_corte_parcial.corte_folio : "AUN SIN CORTES";
                lbl_empleado_ultimo_corte_parcial.Text = (dto_ultimo_corte_parcial.nombre_empleado.Equals("")) ? "AUN SIN CORTES" : dto_ultimo_corte_parcial.nombre_empleado;
            }
            else
            {
                lbl_fecha_ultimo_corte.Text = "AUN SIN CORTES";
                lbl_folio_ultimo_corte.Text = "AUN SIN CORTES";
                lbl_empleado_ultimo_corte_parcial.Text = "AUN SIN CORTES";
            }
        }

        public void get_informacion_corte_total()
        {
            dto_corte_total = dao_cortes.get_corte_total();

            lbl_iva_ct.Text = dto_corte_total.importe_iva.ToString("C2");
            lbl_ieps_ct.Text = dto_corte_total.importe_ieps.ToString("C2");
            lbl_excento_ct.Text = dto_corte_total.importe_excento.ToString("C2");
            lbl_gravado_ct.Text = dto_corte_total.importe_gravado.ToString("C2");
            lbl_venta_bruta_ct.Text = dto_corte_total.importe_bruto.ToString("C2");

            lbl_folio_ventas_total.Text = "Ventas del dia del folio #" + dto_corte_total.venta_inicial + " - #" + dto_corte_total.venta_final;

            //prepagos
            lbl_prepago_cancelados_ct.Text = dto_corte_total.importe_prepagado_cancelado.ToString("C2");
            //lbl_prepagos_canjeados_ct.Text = dto_corte_total.importe_prepagado_canjeado.ToString("C2");
            lbl_prepagos_realizados_ct.Text = dto_corte_total.importe_prepagado.ToString("C2");

            //descuentos
            //lbl_descuento_excento_ct.Text = "-" + dto_corte_total.importe_descuento_excento.ToString("C2");
            //lbl_descuento_gravado_ct.Text = "-" + dto_corte_total.importe_descuento_gravado.ToString("C2");
            lbl_cancelados_ct.Text = dto_corte_total.importe_cancelaciones.ToString("C2");
            //enddescuentos

            lbl_ventas_totales_ct.Text = (dto_corte_total.importe_bruto - dto_corte_total.importe_cancelaciones).ToString("C2");
            //lbl_subtotal_ct.Text = ((dto_corte_total.importe_gravado + dto_corte_total.importe_excento) - (dto_corte_total.importe_descuento_excento + dto_corte_total.importe_descuento_gravado)).ToString("C2");

            lbl_total_parcial_ct.Text = dto_corte_total.importe_total.ToString("C2");

            lbl_vales_emitidos_ct.Text = dto_corte_total.vales_emitidos.ToString("C2");
            lbl_total_mas_vales_ct.Text = Convert.ToDecimal((dto_corte_total.importe_total + dto_corte_total.vales_emitidos + dto_corte_total.importe_prepagado) - dto_corte_total.importe_prepagado_cancelado).ToString("C2");

            dto_ultimo_corte_total = dao_cortes.get_ultimo_corte(true);

            if (dto_ultimo_corte_total.corte_id > 0)
            {
                lbl_fecha_ultimo_corte_total.Text = (dto_ultimo_corte_total.fecha != null) ? Misc_helper.fecha(dto_ultimo_corte_total.fecha.ToString(), "LEGIBLE") : "AUN SIN CORTES";
                lbl_folio_ultimo_corte_total.Text = (dto_ultimo_corte_total.corte_id > 0) ? "#" + dto_ultimo_corte_total.corte_folio : "AUN SIN CORTES";
                lbl_empleado_ultimo_corte_total.Text = (dto_ultimo_corte_total.nombre_empleado.Equals("")) ? "AUN SIN CORTES" : dto_ultimo_corte_total.nombre_empleado;
            }
            else
            {
                lbl_fecha_ultimo_corte_total.Text = "AUN SIN CORTES";
                lbl_folio_ultimo_corte_total.Text = "AUN SIN CORTES";
                lbl_empleado_ultimo_corte_total.Text = "AUN SIN CORTES";
            }

        }

        private void Corte_total_principal_Load(object sender, EventArgs e)
        {
            get_informacion_corte();
            get_informacion_corte_total();
            this.get_hora_corte();
        ...
        private void get_hora_corte()
        {
            DateTime Hoy = DateTime.Today;
            string fecha_actual = Hoy.ToString("dddd", new CultureInfo("es-ES")).ToUpper();
            int horainicialpermitida = 0;
            int horafinalpermitida = 0;
            string HorasSa = "";


            int horactual = Convert.ToInt32(DateTime.Now.ToString("HHmm"));
            ...
            switch (fecha_actual)
            {
                case "SABADO":
                    ...
                    HorasSa = HELPERS.Config_helper.get_config_local("horacortesabado");

                    string[] horarios = HorasSa.Split('-');

                    //0 -inicial
                    //1 -final
                    horainicialpermitida = Convert.ToInt32(horarios[0]);
                    horafinalpermitida = Convert.ToInt32(horarios[1]);



                    break;
                case "DOMINGO":
                    HorasSa = HELPERS.Config_helper.get_config_local("horacortedomingo");
                    string[] horariosdomingo = HorasSa.Split('-');

                    //0 -inicial
                    //1 -final
                    horainicialpermitida = Convert.ToInt32(horariosdomingo[0]);
                    horafinalpermitida = Convert.ToInt32(horariosdomingo[1]);


                    break;
                default:
                    horainicialpermitida = Convert.ToInt32(HELPERS.Config_helper.get_config_local("horacorteinicial"));

                    horafinalpermitida = Convert.ToInt32(HELPERS.Config_helper.get_config_local("horacortefinal"));

                    break;

            }

            /*Revisa la hora para generar el corte*/
            if (horactual > horainicialpermitida && horactual <= horafinalpermitida)
            {
                btn_generar_corte.Show();
            }
        ...
        {
            Login_form login = new Login_form("generar_corte_total");
            login.ShowDialog();

            if (login.empleado_id != null)
            {
                DialogResult dr = MessageBox.Show(this, "¿Esta seguro de querer generar el corte?", "Corte Total", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2);
                ...
                if (dr == DialogResult.Yes)
                {
                    ...
                    */
                    ...
                    {
                        foreach (DataRow dr_entrada in entradas_pendientes.Rows)
                        {
                            folios_pendientes += "#" + dr_entrada["entrada_id"] + ",";
                    ...
                    if (hacerCorte)
                    {
                        var val = dao_cortes.generar_corte_total((long)login.empleado_id, dto_corte_total);
                        ...
                        if (val.status)
                        {
                            ...
                            //RESUMEN
                            resumen_operaciones resumen = new resumen_operaciones();
                            resumen.construccion_ticket();
                            resumen.print();

                        }

                        MessageBox.Show(this, val.informacion, (val.status) ? "Corte Total" : "Error", MessageBoxButtons.OK, (val.status) ? MessageBoxIcon.Information : MessageBoxIcon.Error);
        ...
        private void btn_reimprimir_total_Click(object sender, EventArgs e)
        {
            if (dto_ultimo_corte_total.corte_id > 0)
            {
                Cortes corte = new Cortes();
                corte.construccion_ticket(dto_ultimo_corte_total.corte_id, true);
                corte.print();
            }
            else
            {
                MessageBox.Show(this, "Caja aun sin cortes totales", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        private void btn_reimprimir_parcial_Click(object sender, EventArgs e)
        {
            if (dto_ultimo_corte_parcial.corte_id > 0)
            {
                Cortes corte = new Cortes();
                corte.construccion_ticket(dto_ultimo_corte_parcial.corte_id, true);
                corte.print();
            }
            else
            {
                MessageBox.Show(this, "Caja aun sin cortes parciales", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        private void Corte_total_principal_Shown(object sender, EventArgs e)
        {
