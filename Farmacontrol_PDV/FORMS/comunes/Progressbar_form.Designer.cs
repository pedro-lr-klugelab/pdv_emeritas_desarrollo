namespace Farmacontrol_PDV.FORMS.comunes
{
	partial class Progressbar_form
	{
		/// <summary>
		/// Required designer variable.
		/// </summary>
		private System.ComponentModel.IContainer components = null;

		/// <summary>
		/// Clean up any resources being used.
		/// </summary>
		/// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
		protected override void Dispose(bool disposing)
		{
			if (disposing && (components != null))
			{
				components.Dispose();
			}
			base.Dispose(disposing);
		}

		#region Windows Form Designer generated code

		/// <summary>
		/// Required method for Designer support - do not modify
		/// the contents of this method with the code editor.
		/// </summary>
		private void InitializeComponent()
		{
			this.barra = new System.Windows.Forms.ProgressBar();
			this.label_mensaje = new System.Windows.Forms.Label();
			this.SuspendLayout();
			// 
			// barra
			// 
			this.barra.Location = new System.Drawing.Point(12, 30);
			this.barra.Name = "barra";
			this.barra.Size = new System.Drawing.Size(426, 20);
			this.barra.TabIndex = 0;
			// 
			// label_mensaje
			// 
			this.label_mensaje.Location = new System.Drawing.Point(12, 9);
			this.label_mensaje.Name = "label_mensaje";
			this.label_mensaje.Size = new System.Drawing.Size(426, 13);
			this.label_mensaje.TabIndex = 1;
			this.label_mensaje.Text = "TEXTO";
			// 
			// Progressbar_form
			// 
			this.AutoScaleDimensions = new System.Drawing.SizeF(6F, 13F);
			this.AutoScaleMode = System.Windows.Forms.AutoScaleMode.Font;
			this.ClientSize = new System.Drawing.Size(450, 67);
			this.ControlBox = false;
			this.Controls.Add(this.label_mensaje);
			this.Controls.Add(this.barra);
			this.FormBorderStyle = System.Windows.Forms.FormBorderStyle.FixedSingle;
			this.Name = "Progressbar_form";
			this.ShowInTaskbar = false;
			this.StartPosition = System.Windows.Forms.FormStartPosition.CenterScreen;
			this.Text = "Procesando...";
			this.TopMost = true;
			this.KeyDown += new System.Windows.Forms.KeyEventHandler(this.Progressbar_form_KeyDown);
			this.ResumeLayout(false);

		}

		#endregion

		public System.Windows.Forms.ProgressBar barra;
		public System.Windows.Forms.Label label_mensaje;

	}
}